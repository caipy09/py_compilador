...
#                     L   D   =  \n  "" EOF 
    ...
    def _function_matrix_generator(self):
        return [
            [self.f1, self.f2, self.fn, self.fn, self.fn, self.fn], #inicial
            [self.f3, self.f3, self.f4, self.f4, self.f4, self.f4], #ID
            [self.f5, self.f6, self.f5, self.f5, self.f5, self.f5]
        ]+[[self.fn] * 6] * 1
    
    def __init__(self, filename = None):
        self.token_list = pd.DataFrame()
        self.token_list["nombre"] = ["ID", "CTE", "ASIG"]
        self.token_list["valor"] = [None, None, "="]

        self.ts = st.SymbolTable("DummyLexer EV S")
  ...
  #funcion para hacer unreads
    ...
    def f5(self, c):
        ...
        #     pass
        # else:
        self.ts.addSymbol(self.cache, self.cache, len(self.cache), None)
    ...
    def token(self):
        ...
        if(tokenId != None):
            ...
            tokenType = (self.token_list[self.token_list["nombre"] == tokenId]["nombre"]).iloc[0]
            #print("ca")
            if(tokenId != "ASIG"):
                #print("ca1")
                tokenValue = self.ts.getSymbolByName(self.cache)["valor"].iloc[0]
            else:
                #print("ca3")
                tokenValue = (self.token_list.loc[self.token_list["nombre"] == tokenId]["valor"]).iloc[0]
            #teniendo todos los atributos del token definidos se procede a crear el objeto token y devolverlo
            #print("d1")
            return lt.LexToken(tokenType, tokenValue)
        else:
            #print("d2")
